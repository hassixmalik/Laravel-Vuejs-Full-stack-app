...
const form = useForm<OrderPayload>({
  ...
  placed_by_name: page.props.order.placed_by_name,
...
const customerName = computed(() => {
  const opt = page.props.customerOptions.find(o => o.value === form.customer_id)
  return opt?.label ?? `#${form.customer_id}`
})
...
function convertToInvoice() {
  ...
  router.post(`/orders/${form.id}/convert-to-invoice`, {}, {
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-3">
      ...
      <div v-if="isFulfilled" class="my-3 rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800 dark:bg-neutral-700 dark:text-neutral-100">
        This order is <span class="font-semibold">fulfilled</span> and can’t be edited.
        Create new order?
        <Link href="/orders/createOrder" class="underline">Click here</Link>.
      </div>

      <!-- READ-ONLY CARD when fulfilled -->
      <div v-if="isFulfilled" class="dark:bg-neutral-700 dark:text-neutral-100 rounded-lg border bg-white p-4 space-y-4">
        <div class="flex items-center justify-between">
          <h3 class="text-lg font-semibold">Order #{{ form.id }}</h3>
          <span class="text-xs rounded px-2 py-1" :class="{ 'bg-green-100 text-green-700': true }">
        ...
        <div class="grid grid-cols-3 gap-4 text-sm">
          ...
          <div>
            <div class="text-gray-500">Placed By</div>
            <div class="font-medium">{{ page.props.placedByName }}</div>
          </div>
          <div>
            <div class="text-gray-500">Items</div>
            <div class="font-medium">{{ form.items.length }}</div>
          </div>
          <div>
            <div class="text-gray-500">Total</div>
            <div class="font-semibold tabular-nums">{{ computedTotal.toFixed(3) }}</div>
          </div>
        </div>

        <div class="border-t pt-3 flex items-center justify-between">
          <div class="text-xs text-gray-500">
            You can convert this order into an invoice.
          </div>
          <Button :disabled="converting" @click="convertToInvoice">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="grid grid-cols-12 gap-3" v-for="(it, idx) in form.items" :key="idx">
              ...
              <div class="col-span-3">
                ...
                <Input type="number" min="0" step="0.001" :value="Number(it.unit_total ?? 0).toFixed(3)"
                  :placeholder="Number(it.unit_total ?? 0).toFixed(3)" disabled />
          ...
          <div class="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Input type="text" :value="page.props.order.placed_by_name" :placeholder="page.props.order.placed_by_name" disabled />
